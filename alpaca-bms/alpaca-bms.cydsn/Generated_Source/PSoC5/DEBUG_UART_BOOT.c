...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_DEBUG_UART) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        DEBUG_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != DEBUG_UART_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = DEBUG_UART_GetRxBufferSize();
                CyDelay(DEBUG_UART_BYTE2BYTE_TIME_OUT);
            }
            while(DEBUG_UART_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(DEBUG_UART_GetRxBufferSize() > 0u)
    {
        tempCount = DEBUG_UART_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = DEBUG_UART_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional time DEBUG_UART_WAIT_EOP_DELAY.
            */
            if(pData[dataIndexCntr - 1u] != DEBUG_UART_PACKET_EOP)
            {
                CyDelay(DEBUG_UART_WAIT_EOP_DELAY);
            }
        }
